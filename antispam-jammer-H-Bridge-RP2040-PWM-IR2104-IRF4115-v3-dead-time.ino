...
void update_frequency_safe(uint32_t freq) {
    ...
    // DEAD TIME will last 25% of full cycle and will ensure that 
    // LC circuit transducers + coil unload the high voltage 
    // Channel A will be ON for 0.5 * wrap ( which is 25% of total 2*wrap).
    // Channel B will be ON for 0.5 * wrap (on top thanks to signal negation ).
