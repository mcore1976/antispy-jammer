...
void update_frequency_safe(uint32_t freq) {
    ...
    uint16_t level = (uint16_t)((wrap + 1) * random(65,70) / 100);
...
void loop() {
    ...
    while (i >= 24000) {
        ...
        // Randomized frequency jumps but small intervals 
        // give the best jamming for newer phones
           j = (j + random(1,4) ) % x;
                   
        if (j == 0) j = 1;  // safeguard
       
       // you may want to adjust these
       // when this value is below 50usec jammer becomes audible
       // initially it was 140usec
