/*
    ...
    CONNECTION EXAMPLE FOR ARDUINO PRO MINI
    --------
    AD9833 PCB to Raspberry Pi Pico RP2040-ZERO module :
    SDATA --> pin 3 MOSI (GP3)
    SCLK --> pin 2 SCK (GP2)
    FSYNC --> pin 1 (GP1) - adjust this pin for you Arduino board down in the code FNC_PIN
    DGND --> GND
    VCC --> 5V pin 
...
static inline void ad9833_send(uint16_t packet)
{
    ...
    */
    gpio_put(FNC_PIN,0); // set cs_pin to LOW
  ...
  // TODO: Are we running at the highest clock rate?
    ...
    /*
    digitalWrite(FNC_PIN,HIGH);   // FNCpin HIGH  
    */
    gpio_put(FNC_PIN,1); // set cs_pin to HIGH
...
void setup() 
  ...
  */   
  gpio_init(FNC_PIN); 
  gpio_set_dir(FNC_PIN, GPIO_OUT);  // set cs_pin to output
  gpio_put(FNC_PIN,1);              // set cs_pin initially to high
      
  // setup SPI pins for RP2040-ZERO board
  SPI.setRX(0);
  SPI.setCS(1);
  SPI.setSCK(2);
  SPI.setTX(3);
  SPI.begin(false);  // DISABLE HW based CS/FSYNC signal - it will be manually handled
  SPI.begin(); 

  // special settings for RP2040
...
void loop() {
     ...
     uint16_t i,j,x;
          ...
          // going down with frequency from j to 24kHz
          while (i >= 24000)
          {
           i = i - j;
          ...
          // now the opposite direction going up to 26000 
