...
const uint8_t  randompwm[] PROGMEM = 
  {
...
const uint8_t  randomampl[] PROGMEM = 
  {
22,19,31,30,26,21,24,11,21,28,17,21,29,7,15,8,24,22,20,6,27,15,27,
22,5,24,24,14,24,23,29,19,16,30,15,24,20,9,18,17,26,28,10,10,17,18,
16,23,16,7,6,30,10,30,9,12,18,12,15,14,28,20,11,10,19,23,28,7,13
};

int main(void)
{
        uint16_t sequence;
        uint8_t pulsewidth, amplitude, i, bulk;
       ...
       { 
          ...
          for(sequence=0; sequence<800; sequence++)
           ...
           pulsewidth = pgm_read_byte(randompwm + sequence);

           // take random amplitude from lookup table
           // so we bit shift right the counter - div 16
           amplitude =  pgm_read_byte(randomampl + (sequence >> 0x04));

             // sending a bulk of pulses with defined pulsewidth
             // you may play with this value for better effect
             for(bulk=0; bulk<25; bulk++)
             {
              // send amplitude from lookup table to DAC
              ...
              OUTPUTPORT = 0; 
              // now delay to achieve desired frequency
              // you may play with this value '47' to achieve 25kHz
